...
def _collapse_deliverables(items: list[str]) -> list[str]:
    collapsed = []
    report_group = []
    inside_report_group = False
    ...
    for raw in items:
        ...
        if item.startswith("5. Un informe final en markdown que resuma:"):
            inside_report_group = True
            report_group = []
            continue

        if inside_report_group and item.startswith("6. "):
            if report_group:
                collapsed.append(
                    "5. Un informe final en markdown que incluya: " + "; ".join(report_group)
                )
            inside_report_group = False
            report_group = []
        ...
        if inside_report_group:
            report_group.append(item)
            continue

        collapsed.append(item)

    if inside_report_group and report_group:
        collapsed.append(
            "5. Un informe final en markdown que incluya: " + "; ".join(report_group)
        )
...
def interpret_goal():
    ...
    deliverables = _collapse_deliverables([item for item in sections["deliverables"] if item])
